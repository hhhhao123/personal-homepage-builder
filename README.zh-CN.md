...
- **动代码前要有授权快照**：真正修改文件前，Agent 需要说明已确认的方案、目标文件、占位内容、GitHub Pages 假设和验证方式，并等待确认。
- **调整也要先确认**：即使只是优化已有页面、添加动效、改栏目或调整样式，也应该先讨论方案，用户确认后再改代码。
- **初版不满意也有迭代路线**：如果第一版主页太普通、不像自己、元素不好看或动效不合适，Agent 应该先诊断问题，再选择性调用必要的辅助 Skill，确认改版方案后再改代码。
...
- **交付前对照方案检查**：最终页面需要回看已确认的主页方案，确认身份、受众、栏目、页面结构、动效、隐私和占位内容没有跑偏。
...
- 第一版之后的视觉诊断和改版方案，比如风格不对、元素不好看、动效不合适、参考网站需要转化
- 对已有主页的局部优化建议
- 已有页面调整前的改动方案和确认步骤
